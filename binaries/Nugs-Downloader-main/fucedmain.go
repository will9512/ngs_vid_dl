...
func downloadVideo(videoPath string, segUrls []string, done chan bool) {
...
func downloadSegment(segURL string, startByte int64, outputFile *os.File, done chan bool) {
    ...
    // Loop through segment URLs and start Goroutines for each segment
    for _, segURL := range segUrls {
        go downloadSegment(segURL, startByte, outputFile, done)
    }

    // Wait for all Goroutines to complete
    for i := 0; i < len(segUrls); i++ {
        success := <-done
        if !success {
            fmt.Println("Error downloading segment")
            // Handle the error
        }
    }
...
func downloadTrack(trackPath, trackURL string, resultChan chan<- DownloadResult) {
	...
	outputFile, err := os.Create(trackPath)
	if err != nil {
		result := DownloadResult{
			TrackPath: trackPath,
			Error:     err,
...
func main() {
	...
	// Create a channel to signal completion of Goroutines for segment downloads
	segDone := make(chan bool)

	// Loop through segment URLs and start Goroutines for each segment
	for _, segUrl := range segUrls {
		go downloadVideo(videoPath, segUrl, segDone)
	}

	// Wait for all segment Goroutines to complete
	for i := 0; i < len(segUrls); i++ {
		success := <-segDone
		if !success {
			fmt.Println("Error downloading segment")
	...
	// Create a channel to signal completion of Goroutines for track downloads
	trackDone := make(chan bool)

	// Loop through track URLs and start Goroutines for each track
	for _, trackURL := range trackURLs {
		go func(url string) {
			err := downloadTrack(trackPath, url, trackDone)
			if err != nil {
				fmt.Printf("Error downloading track: %v\n", err)
				// Handle the error
			} else {
				fmt.Printf("Successfully downloaded track: %s\n", url)
			}
		}(trackURL)
	}

	// Wait for all track Goroutines to complete
	for i := 0; i < len(trackURLs); i++ {
		<-trackDone
	}
